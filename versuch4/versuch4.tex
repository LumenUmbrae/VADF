...
\begin{equation}
	\label{eq:ampere}
	\int_{\partial A} \vec{H} \cdot d\vec{s} = \int_A \vec{J} \cdot d \vec{A} 
\end{equation}
ergibt sich durch Transformation der linken Seite in Zylinderkoordinaten und auflösen der rechten Seite
\begin{equation*}
	\int_{0}^{2\pi} \vec{H} \vec{e_\varphi} \cdot r \ d \varphi = I
\end{equation*}.
Durch auflösen des Integrales, umstellen nach $\vec{H}$, umformen von $\vec{e_\varphi} = \vec{e_x} sin(\varphi) + \vec{e_y} cos(\varphi)$ und einsetzen in das Materialgesetz $\vec{H} = \mu \vec{B}$  ergibt sich somit
\begin{equation}
	\label{eq:bLeiter}
	\vec{B} = \frac{I}{2\mu \pi r} \vec{e_\varphi}  % wie geben ich hier den Richtungsvektor an????
\end{equation}
\emph{Fügen Sie hier Ihre Lösung ein}
...
Wie in Abb. \ref{fig:residiumiIterationen} zu erkennen nimmt das relative Residuum expotenzell mit der Anzahl der Iterationen ab. 
Bim relativen Risidium Handelt es sich um die Differenz zwischen der $i.$ und $(i-1).$ Iteration. 

\begin{figure}
	\centering
	\includegraphics[width=0.7\linewidth]{versuch4/ResidiumIterationenGraph}
...
\begin{figure}
	\centering
	\includegraphics[width=0.7\linewidth]{versuch4/KapazitaetUeberAnzStuetzstellen}
